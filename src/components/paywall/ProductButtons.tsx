    ...
    <div className="flex w-full max-w-sm flex-col items-center gap-3">
      ...
      <div className="mb-2 flex flex-col items-center gap-2">
        <div className="relative">
          <div className="absolute inset-0 rounded-full bg-[var(--theme-primary)]/20 blur-xl" aria-hidden="true" />
          <div className="relative flex h-12 w-12 items-center justify-center rounded-full bg-[var(--theme-primary)] text-[var(--theme-primary-text)]">
        ...
        <span className="text-xl font-semibold tracking-tight text-[var(--theme-media-text)]">
      ...
      <div className="flex w-full flex-col gap-2.5">
        {products.map((product) => (
          <button
            ...
            disabled={loading}
            className="group relative w-full overflow-hidden rounded-xl border border-[var(--theme-border)] bg-[var(--theme-bg-secondary)] px-4 py-3.5 text-left shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-[var(--theme-primary)] hover:bg-[var(--theme-hover)] disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:translate-y-0"
          >
            <span
              aria-hidden="true"
              className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[var(--theme-border)] to-transparent"
            />
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0 flex-1">
                <span className="block truncate text-sm font-semibold tracking-tight text-[var(--theme-heading)]">
                  {product.name || t("viewer.payment.unlock_content")}
                </span>
                {product.accessDurationSeconds != null && (
                  <span className="mt-1 inline-flex items-center gap-1.5 text-xs text-[var(--theme-text-secondary)]">
                    <svg
                      aria-hidden="true"
                      width="11"
                      height="11"
              ...
              {product.priceCents != null && (
                <span className="shrink-0 text-lg font-semibold tabular-nums tracking-tight text-[var(--theme-link)] transition-transform duration-200 group-hover:scale-105">
      ...
      <button
        ...
        className="group mt-2 inline-flex min-h-11 items-center gap-1.5 text-sm font-medium text-[var(--theme-media-text)]/80 transition-colors hover:text-[var(--theme-media-text)]"
      >
        <span className="underline underline-offset-4 decoration-[var(--theme-media-text)]/40 group-hover:decoration-[var(--theme-media-text)]">
