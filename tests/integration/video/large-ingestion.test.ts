...
import { mkdtemp, rm, statfs, readdir, open, readFile, chmod } from "node:fs/promises";
...
it.skipIf(process.env.VIDEO_LARGE_TEST !== "true")("resumes a 10 GiB HTTP upload across server/client restarts and fences a killed encoder", async () => {
  ...
  try {
    ...
    const source = execFileSync("ffmpeg", ["-v", "error", "-f", "lavfi", "-i", "testsrc2=size=320x180:rate=30", "-t", "8.5", "-c:v", "libx264", "-threads", "2", "-preset", "ultrafast", "-bf", "0", "-movflags", "frag_keyframe+empty_moov+default_base_moof", "-f", "mp4", "pipe:1"], { maxBuffer: PART_BYTES });
    ...
    function part(offset: number) { const data = Buffer.alloc(Math.min(PART_BYTES, total - offset)); if (offset === 0) { source.copy(data); header.copy(data, source.length); } return data; }
    ...
    if (process.env.VIDEO_DOCKER_TEST === "true") {
      ...
      processed = { maxRSSKiB: 0, container: true };
    ...
    expect(files).toBeGreaterThanOrEqual(total / PART_BYTES); expect(await readdir(path.join(root, "tmp"))).toHaveLength(0);
    ...
    process.stdout.write(JSON.stringify({ evidence: "phase2-10gib", bytes: total, objects: files, uploadMs, serverRSSKiB: [firstRSS, secondRSS], workerNodeRSSKiB: processed.maxRSSKiB, attempts: ready.version.job?.attempts, railCalls }) + "\n");
  } finally {
